...
load("GWAS_4_Annotation.RData")

# assign file name and the path of the gds files to an object 
geno.file <- "./GWAS4_geno.gds"
qxy.file <- "./GWAS4_qxy.gds"
bl.file <- "./GWAS4_bl.gds"
...
# 0.0003974 0.0008680 0.0011258 0.0065478 0.0016483 0.7601106 
# Look at missing.e1 for females
summary(scanAnnot$missing.e1[scanAnnot$sex == "F"])
#      Min.   1st Qu.    Median      Mean   3rd Qu.      Max. 
# 0.0004062 0.0008539 0.0010918 0.0078321 0.0015193 0.7700731 
...
summary(missa)
#      Min.   1st Qu.    Median      Mean   3rd Qu.      Max. 
# 0.0003898 0.0008508 0.0010924 0.0072886 0.0015257 0.7708418 

# X chromosome
missx <- miss$missing.counts[,"X"] / miss$snps.per.chr["X"]
summary(missx)
#       Min.   1st Qu.    Median      Mean   3rd Qu.      Max. 
#  0.0002405 0.0007053 0.0009618 0.0079493 0.0014267 0.7962072 
...
d <- Biobase::pData(scanAnnot)

himis <- d[scan.exclude,c("scanName","sex","race",
                          "Case_Control_Baseline", "Case_Control_Current", "Disease_Status", "missing.e1")]

write.table(himis, "./QC_check/High_Missing_Call_Rate_Samples.txt", quote=FALSE, row.names = FALSE, sep="\t")
...
#      Min.   1st Qu.    Median      Mean   3rd Qu.      Max. 
# 0.0000000 0.0002271 0.0006814 0.0072417 0.0015898 1.0000000
...
#      Min.   1st Qu.    Median      Mean   3rd Qu.      Max. 
# 0.0003001 0.0006810 0.0009290 0.0071482 0.0014181 0.7803089
...
# bn         1 0.0001036 1.0360e-04  1.3121 0.2579
# Residuals 46 0.0036319 7.8954e-05                     
# ---
...
hist(exc.qual.results[,"median.quality"], main="Median Genotype Quality Scores
     of Samples", xlab="Median Quality", las=1, ylim=c(0,4400), xlim=c(0.79, 0.83),
     breaks = seq(0.79, 0.83,0.001), axes=FALSE)
axis(1, at=seq(0.79, 0.83,0.001), cex.axis=0.7, las=2)
axis(2, at=seq(0,4400,4400), cex.axis=0.7, las=1)
text(0.823, 4400, paste0("Min = ", round(min(exc.qual.results[,"median.quality"]),4))) 
dev.off()
...
mean_inten <- merge(Biobase::pData(scanAnnot), mninten2, by="scanID", all=TRUE)
...
inten.by.chrom.noexclu <- meanIntensityByScanChrom(qxyData, snp.exclude = NULL)
# names(inten.by.chrom)

mninten.noexclu <- inten.by.chrom.noexclu[[1]]  # mean intensities
# dim(mninten)

### Check to be sure sample ordering is consistent
mninten2.noexclu <- as.data.frame(mninten.noexclu[order(as.integer(rownames(mninten.noexclu))),])
allequal(scanAnnot$scanID, rownames(mninten2.noexclu))
# TRUE
mninten2.noexclu$scanID <- row.names(mninten2.noexclu)

# Merge the annotation file and the mean intensity file
mean_inten.noexclu <- merge(Biobase::pData(scanAnnot), mninten2.noexclu, by="scanID", all=TRUE)

pdf("./QC_check/Ch5.1-2 Mislabeled-Sex check by plate.pdf")
par(mfrow=c(2,2))

uniq_plates <- sort(unique(as.integer(mean_inten.noexclu$plate)), 
                    decreasing = FALSE)
for(i in uniq_plates){
  
  # Assign each sex a color for each plate
  temp <- subset(mean_inten.noexclu, plate==i)
...
#####################################################################
# inten.by.chrom_all <- meanIntensityByScanChrom(qxyData, snp.exclude = NULL)
# names(inten.by.chrom_all)

# mninten_all <- inten.by.chrom_all[[1]]  # mean intensities
# dim(mninten)

### Check to be sure sample ordering is consistent
# mninten_all2 <- as.data.frame(mninten_all[order(as.integer(rownames(mninten_all))),])
# allequal(scanAnnot$scanID, rownames(mninten_all2))
# TRUE
# mninten_all2$scanID <- row.names(mninten_all2)

# Merge the annotation file and the mean intensity file
# mean_inten_all_all <- merge(Biobase::pData(scanAnnot), mninten_all2, by="scanID", all=TRUE)
...
#All intensities
x1 <-mean_inten.noexclu[,"X"] 
y1 <- mean_inten.noexclu[,"Y"]
main1 <- "Mean X vs \nMean Y Chromosome Intensity"
#Het on X vs X intensity
x2 <- mean_inten.noexclu[,"X"]
y2 <- scanAnnot$het.X
main2 <- "Mean X Chromosome Intensity vs
Mean X Chromosome Heterozygosity"
# Het on X vs Y intensity
y3 <- mean_inten.noexclu[,"Y"]
x3 <- scanAnnot$het.X
...
ind1 <- which(mean_inten.noexclu$X>1 & mean_inten.noexclu$sex=="M")
ind2 <- which(mean_inten.noexclu$Y>0.5 & mean_inten.noexclu$sex=="F")

ind <- sort(c(ind1, ind2))

mis_sex <- subset(mean_inten.noexclu[ind,], select=c("scanName","sex","race",
                                                 "Case_Control_Baseline", "Case_Control_Current", "Disease_Status","file", "plate", "well_position"))

write.table(mis_sex[order(mis_sex$plate, mis_sex$well_position),], "./QC_check/Mislabeled Sex IDs.txt", quote=FALSE, row.names = FALSE, sep="\t")

###

ind3 <- which(mean_inten.noexclu$het.X < 0.05 & mean_inten.noexclu$sex=="F")
ind4 <- ind3[ind3%nin%ind]

mis_1x <- subset(mean_inten.noexclu[ind4,], select=c("scanName","sex","race",
                                                 "Case_Control_Baseline", "Case_Control_Current", "Disease_Status", "file", "plate", "well_position"))

write.table(mis_1x[order(mis_1x$plate, mis_1x$well_position),], "./QC_check/X singelton female.txt", quote=FALSE, row.names = FALSE, sep="\t")
...
###################################################
samp <- Biobase::pData(scanAnnot)[,c("scanID", "scanName")]
...
#  18197 46945    71   158   248 27034 

table(ibd$exp.rel, ibd$obs.rel, useNA="ifany")
#    Deg2  Deg3   Dup    FS    PO     U
# U 18197 46945    71   158   248 27034
...
write.table(ibd2, "./QC_check/GWAS_4_Kinship_Estimate.txt", quote=FALSE, row.names = FALSE, sep="\t")
...
samp <- Biobase::pData(scanAnnot)[match(pca$sample.id, scanAnnot$scanID),]
cols <- rep(NA, nrow(samp))

table(samp$race)
samp$re_race <- ifelse(samp$race == "W", "White", 
                       ifelse(samp$race == "B", "Black", 
                              ifelse(samp$race %in% c("A", "AIAN"), "Asian",
                                     ifelse(samp$race %in% c("Multi-Racial", "W/AIAN", "W/B", "W/B", "W/B/AIAN"), "Multi-Racial",
                                            ifelse(samp$race %in% c("-1", "-12", "-11", "-4", "-11"), "Unknown", "Other")))))
...
cols[samp$re_race == "Multi-Racial"] <- "orange"
...
pairs(pca$eigenvect[,1:4], col=cols, labels=lbls,
      main = "White: blue, Black: red, Asian: green, Others: black, 
      Multi-Racial: orange, Unknown: grey")
dev.off()
...
plot(c(0,15), c(0,1), type = 'n', axes = FALSE, ylab = "", xlab = "",
     ...
     White: blue, Black: red, Asian: green, Others: black, 
     Multi-Racial: orange, Unknown: grey")
for (j in 1:13)
...
samples.nodup <- Biobase::pData(scanAnnot)[sample.sel,]

# combine scanID, case/control, and race information
princomp$scanID <- as.factor(samples.nodup$scanID)
princomp$case.ctrl.status <- as.factor(samples.nodup$Case_Control_Current)
...
table(samples.nodup$Case_Control_Baseline)
table(samples.nodup$Case_Control_Current)

# Assign color codes for AD and Control
cols <- rep(NA, nrow(samples.nodup))
cols[samples.nodup$Case_Control_Current == "Case"] <- "green"
cols[samples.nodup$Case_Control_Current == "Control"] <- "magenta"
...
lm.all <- lm(scanAnnot$missing.e1 ~ scanAnnot$Case_Control_Current)
summary(aov(lm.all))

#                                 Df  Sum Sq   Mean Sq F value Pr(>F)
# scanAnnot$Case_Control_Current    3  0.001 0.0002908    0.15   0.93
# Residuals                      2923  5.670 0.0019397       


# Plot Missing call rate by status
pdf("./QC_check/Ch6.2-1 Missing call rate by status.pdf")
boxplot(scanAnnot$missing.e1 ~ scanAnnot$Case_Control_Current, 
...
loh.filt <- loh.filt[!is.na(loh.filt$scanID),]
str(loh.filt)
# 3 obs 
...
save.image("./QC_check/GWAS_4_QC.RData")
###################################################
### 
###################################################
...
indi1_dups_excl <- c("160024", "A0005", "A3937_205107410095_R06C01", "IGN406_205103230094_R05C01", "m0503", "m0588", "M256", "S0263")
indi2_dups_excl <- d_dup[!(d_dup$Individ2 %in% c("A1471", "A2433_205101590068_R06C01", "A3973", "IGN457", "m0525", "m0748", "S0309")),]$Individ2
# length(unique(c(indi1_dups_excl, indi2_dups_excl)))
# 91

elig_subj <- scanAnnot$scanID[!scanAnnot$duplicated & !(scanAnnot$scanName %in% c(indi1_dups_excl, indi2_dups_excl)) & scanAnnot$missing.e1 < 0.05 ]
# 4318

# sum(is.na(snpAnnot@data$a1))
...
subjAnnot <- scanAnnot[scanAnnot$scanID %in% getScanID(genoGDS),]
subjAnnot@data$scanName <- gsub(" ", "", subjAnnot@data$scanName)
...
vcfWrite(genoData, vcf.file="GWAS_4_raw.vcf", sample.col="scanName",
...
save.image("./QC_check/GWAS_4_QC.RData")
# load("./QC_check/GWAS_4_QC.RData")


# d1 <- merge(ibd[,c("Individ1","ii","exp.rel","obs.rel")], 
#             scanAnnot@data[c("scanName","sex", "race", "Case_Control_Baseline", "Case_Control_Current", "Disease_Status")], 
#             by.x = "Individ1", by.y = "scanName")

# colnames(d1) <- c("Individ1", "ii", "exp.rel", "obs.rel", paste0(colnames(d1)[5:9], "_Ind1"))

# d2 <- merge(ibd[,c("Individ2","ii","exp.rel","obs.rel")], 
#             scanAnnot@data[c("scanName","sex", "race", "Case_Control_Baseline", "Case_Control_Current", "Disease_Status")], 
#             by.x = "Individ2", by.y = "scanName")

# colnames(d2) <- c("Individ2", "ii", "exp.rel", "obs.rel", paste0(colnames(d2)[5:9], "_Ind2"))
...
# table(d$obs.rel)
# d_dup <- d[d$exp.rel != "Dup" & d$obs.rel == "Dup",]
# head(d_dup)
# dim(d_dup)
# # [1] 94 15

# d_dup_sex <- d_dup[d_dup$sex_Ind1 != d_dup$sex_Ind2,]
# d_dup_race <- d_dup[d_dup$race_Ind1 != d_dup$race_Ind2,]

# tmp <- princomp[!(princomp$race %in% c(-1,-11,-12,-4)), ]
# ggplot(tmp, aes(x = V1, y = V2, color = factor(race), shape = factor(race))) + 
#   geom_point(size =3) 

# check1 <- subset(princomp, V1 <0 & V2 > -0.1 & V2 < -0.05)
# check2 <- subset(princomp, V2 < -0.175)

# write.table(d_dup, "d_dup.csv", 
#             col.names = TRUE, row.names = FALSE, quote = FALSE, sep = ",")

# write.table(d_dup_rest, "d_dup_rest.csv", 
#             col.names = TRUE, row.names = FALSE, quote = FALSE, sep = ",")


# d_dup_rest <- d_dup[!(d_dup$Individ1 %in% d_dup_race$Individ1) & !(d_dup$Individ1 %in% d_dup_sex$Individ1), ]
# d_dup_rest <- merge(d_dup_rest, scanAnnot@data[,c("scanName","missing.e1")], 
#                     by.x = "Individ1", by.y = "scanName", all.x = TRUE)

# colnames(d_dup_rest) <- c(colnames(d_dup_rest)[-ncol(d_dup_rest)], "missing.e1_Ind1")

# d_dup_rest <- merge(d_dup_rest, scanAnnot@data[,c("scanName","missing.e1")], 
# by.x = "Individ2", by.y = "scanName", all.x = TRUE)
# colnames(d_dup_rest) <- c(colnames(d_dup_rest)[-ncol(d_dup_rest)], "missing.e1_Ind2")

# d_dup_rest_check1 <- d_dup_rest[d_dup_rest$missing.e1_Ind1 >= d_dup_rest$missing.e1_Ind2,]
# d_dup_rest_check2 <- d_dup_rest[d_dup_rest$missing.e1_Ind1 < d_dup_rest$missing.e1_Ind2,]


# View(as.data.frame(d_dup_rest_check1$Individ1))
# View(as.data.frame(d_dup_rest_check2$Individ2)) 

# gwas4_removed_due_to_duplicates <- unique(setdiff(c(d_dup_rest_check1$Individ1, d_dup_rest_check2$Individ2, 
#                                      d_dup_sex1$Individ1, d_dup_sex2$Individ2, "M256", "M265", "A3530"), c("S344", "S364")))
# write.table(gwas4_removed_due_to_duplicates, "gwas4_removed_due_to_duplicates.txt", col.names = FALSE)
