...
load("GWAS_2_Annotation.RData")

# assign file name and the path of the gds files to an object 
geno.file <- "./GWAS2_geno.gds"
qxy.file <- "./GWAS2_qxy.gds"
bl.file <- "./GWAS2_bl.gds"
...
#  0.001109 0.001778 0.002206 0.003171 0.003101 0.129896  
# Look at missing.e1 for females
summary(scanAnnot$missing.e1[scanAnnot$sex == "F"])
#      Min.   1st Qu.    Median      Mean   3rd Qu.      Max. 
# 0.0009749 0.0018193 0.0023450 0.0033687 0.0032311 0.1495838
...
summary(missa)
#      Min.   1st Qu.    Median      Mean   3rd Qu.      Max. 
# 0.0009769 0.0017484 0.0022266 0.0032258 0.0030967 0.1494885 

# X chromosome
missx <- miss$missing.counts[,"X"] / miss$snps.per.chr["X"]
summary(missx)
#       Min.   1st Qu.    Median      Mean   3rd Qu.      Max. 
#  0.0008966 0.0018650 0.0028513 0.0049490 0.0050570 0.1648196
...
#      Min.   1st Qu.    Median      Mean   3rd Qu.      Max. 
#  0.000000  0.000000  0.000000  0.002917  0.001393  0.962848   
...
#      Min.   1st Qu.    Median      Mean   3rd Qu.      Max. 
# 0.0006209 0.0012192 0.0015654 0.0024933 0.0022649 0.1484758 
...
hist(exc.qual.results[,"median.quality"], main="Median Genotype Quality Scores
     of Samples", xlab="Median Quality", las=1, ylim=c(0,800), xlim=c(0.84, 0.87),
     breaks = seq(0.84, 0.87,0.001), axes=FALSE)
axis(1, at=seq(0.84, 0.87,0.001), cex.axis=0.7, las=2)
...
write.table(ibd2, "./QC_check/GWAS2_GWAS_Kinship_Estimate.txt", quote=FALSE, row.names = FALSE, sep="\t")
...
samp$re_race <- ifelse(samp$race == "W", "White", 
                       ifelse(samp$race == "B", "Black", 
                              ifelse(samp$race == "Multi-Racial", "Multi-Racial", 
...
cols[samp$re_race == "Multi-Racial"] <- "green"
...
pairs(pca$eigenvect[,1:4], col=cols, labels=lbls,
      main = "White: blue, Black: red, Multi-Racial: green, Others: black, Unknown: grey")
dev.off()
...
plot(c(0,15), c(0,1), type = 'n', axes = FALSE, ylab = "", xlab = "",
     ...
     White: blue, Black: red, Multi-Racial: green, Others: black, Unknown: grey")
for (j in 1:13)
...
# scanAnnot$Case_Control   2 0.00004 2.205e-05   0.381  0.683
# Residuals              717 0.04145 5.781e-05                
...
#   race[i] <- scanAnnot$race[scanAnnot$GWAS2ID == subjID[i]][1]
# }
...
elig_subj <- scanAnnot$scanID[!scanAnnot$duplicated & scanAnnot$missing.e1 < 0.05
                              & !(scanAnnot@data$scanName %in% c("g0954", "g0832", "g0909", "g0334", "g0338", "g0343", "g0347"))]
# N=711
...
vcfWrite(genoData, vcf.file="GWAS_2_raw.vcf", sample.col="scanName",
...
save.image("./QC_check/GWAS_2_QC.RData")
# load("./QC_check/GWAS_2_QC.RData")
