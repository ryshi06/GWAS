...
ref <- read.table("GWAS_5_Ref_Overlap.txt", sep="\t", header = TRUE)
...
SN <- read.csv("GWAS_5_scanName.txt", header=FALSE)

# create a text string with all the individual file names
SF <- paste("GWAS_5_Sample_",SN$V1,".txt", sep="")
...
clinical_info <- read.table("/zfs1/kfan/Ruyu/harmonization_Sep5/GWAS_Reference_Data/GWAS_ALL_2023_07_24.csv",
                            sep = ",", header = TRUE)
GWAS5_info <- clinical_info[clinical_info$GWAS_5 == "Y", c("AlzID", "STudyID", "LabID", "Sex", "Race", "Case_Control", "Disease_Status", "APOE", "GWAS_5")]

# head(GWAS5_info)
# d1 <- d[tolower(d$scanName) %in% tolower(GWAS5_info$STudyID),]
# d2 <- d[tolower(d$scanName) %in% tolower(GWAS5_info$LabID),]
# setdiff(tolower(d$scanName), tolower(GWAS5_info$LabID))

GWAS5_info$LABID <- toupper(GWAS5_info$LabID)
dd <- merge(d, GWAS5_info, by.x = "scanName", by.y = "LABID", all.x = TRUE)
...
plate_info <- read.table("/zfs1/kfan/Ruyu/harmonization_Sep5/GWAS_Reference_Data/2023-02-06_DNA_Manifest-Kamboh-GWAS5.csv", 
                          sep = ",", header = TRUE, skip = 5)
colnames(plate_info)
GWAS5_plate <- plate_info[, c("Institute.Plate.Label", "Well", "Institute.Sample.Label")]
colnames(GWAS5_plate) <- c("plate", "well", "scanName")

# length(intersect(GWAS5_plate$Institute.Sample.Label, dd$scanName))
d <- merge(dd, GWAS5_plate, by = "scanName", all.x=TRUE)
# colnames(d)
...
save.image("GWAS_5_Annotation.RData")
