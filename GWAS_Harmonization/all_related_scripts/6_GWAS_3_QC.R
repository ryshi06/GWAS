...
load("GWAS_3_Annotation.RData")

# assign file name and the path of the gds files to an object 
geno.file <- "./GWAS3_geno.gds"
qxy.file <- "./GWAS3_qxy.gds"
bl.file <- "./GWAS3_bl.gds"
...
#  0.0003326 0.0010213 0.0015631 0.0106630 0.0035450 0.4383886 
# Look at missing.e1 for females
summary(scanAnnot$missing.e1[scanAnnot$sex == "F"])
#      Min.   1st Qu.    Median      Mean   3rd Qu.      Max. 
# 0.0003908 0.0009856 0.0015413 0.0082778 0.0035818 0.3352907
...
summary(missa)
#      Min.   1st Qu.    Median      Mean   3rd Qu.      Max. 
# 0.0003294 0.0009785 0.0015293 0.0092040 0.0035174 0.4345370 

# X chromosome
missx <- miss$missing.counts[,"X"] / miss$snps.per.chr["X"]
summary(missx)
#       Min.   1st Qu.    Median      Mean   3rd Qu.      Max. 
#  0.0002178 0.0007919 0.0013067 0.0113320 0.0034647 0.6004078 
...
himis <- d[scan.exclude,c("scanName","sex","race","AlzID","STudyID","LabID", 
                          "Case_Control", "Disease_Status", "APOE", "missing.e1")]
...
#      Min.   1st Qu.    Median      Mean   3rd Qu.      Max. 
#  0.000000  0.000000 0.001626   0.003733  0.003252  0.753906  
...
#      Min.   1st Qu.    Median      Mean   3rd Qu.      Max. 
# 0.0002145 0.0006132 0.0009401 0.0085176 0.0023391 0.4381921
...
hist(exc.qual.results[,"median.quality"], main="Median Genotype Quality Scores
     of Samples", xlab="Median Quality", las=1, ylim=c(0,800), xlim=c(0.82, 0.85),
     breaks = seq(0.82, 0.85,0.001), axes=FALSE)
axis(1, at=seq(0.82, 0.85,0.001), cex.axis=0.7, las=2)
axis(2, at=seq(0,800,800), cex.axis=0.7, las=1)
text(0.823, 800, paste0("Min = ", round(min(exc.qual.results[,"median.quality"]),4))) 
dev.off()
...
# lack X,Y information in the original sample file
...
# scanAnnot2 <- Biobase::pData(scanAnnot)[-scan.exclude,]
# scanAnnot2 <- Biobase::pData(scanAnnot)
...
#                                             "Case_Control", "Disease_Status","APOE","file"))
...
# 1156 3381    2   20   14 1363 

table(ibd$exp.rel, ibd$obs.rel, useNA="ifany")
#          Deg2 Deg3  Dup   FS   PO    U
#        U 1156 3381    2   20   14 1363
...
write.table(ibd2, "./QC_check/GWAS3_GWAS_Kinship_Estimate.txt", quote=FALSE, row.names = FALSE, sep="\t")
...
samp$re_race <- ifelse(samp$race == "W", "White", 
                       ifelse(samp$race == "B", "Black", 
                              ifelse(samp$race == "A", "Asian", 
                                     ifelse(samp$race %in% c("-12", "-11", "-4", "0"), "Unknown", "Other"))))
...
# scanAnnot$Case_Control   2 0.0338 0.01688   15.93 1.78e-07 ***
# Residuals              637 0.6751 0.00106                                  
...
# anoms is NULL, no anomlies detected
...
#   race[i] <- scanAnnot$race[scanAnnot$GWAS3ID == subjID[i]][1]
# }
...
# men.list <- with(Biobase::pData(scanAnnot), mendelList(family, subjectID,
...
# #     Mendelian error rate
# fams <- Biobase::pData(scanAnnot)[!is.na(scanAnnot$mend.err.rate.fam) &
...
# ###################################################
# head(Biobase::pData(scanAnnot)[,c("father", "mother")])
...
# snp <- Biobase::pData(snpAnnot)[snp.sel, c("snpID", "rsID")]
...
# identified as duplicates with genotype data, with exactly same phenotype info with the corresponding pair
elig_subj <- scanAnnot$scanID[!scanAnnot$duplicated & !(scanAnnot$scanName %in% c("6205240", "6207537")) & scanAnnot$missing.e1 < 0.05 ]
# length(elig_subj)

missingRef <- is.na(snpAnnot@data$Ref)
snp.exclude <- hla | xtr | gap | ignore | missingRef
...
ref.allele=snpsAnnot@data$Ref
alt.allele=snpsAnnot@data$Alt

vcfWrite(genoData, vcf.file="GWAS_3_raw.vcf", sample.col="scanName",
         id.col="snpName", qual.col=NULL, filter.cols=NULL,
         info.cols=NULL, scan.exclude=NULL, snp.exclude=NULL,
         scan.order=NULL, ref.allele=snpsAnnot@data$Ref, block.size=1000, verbose=TRUE)

save.image("./QC_check/GWAS_3_QC.RData")
# load("./QC_check/GWAS_3_QC.RData")

# d1 <- merge(ibd[,c("Individ1","ii","exp.rel","obs.rel")], 
# scanAnnot2[c("scanName","sex", "race", "Case_Control", "Disease_Status", "APOE")], 
# by.x = "Individ1", by.y = "scanName")

# colnames(d1) <- c("Individ1", "ii", "exp.rel", "obs.rel", paste0(colnames(d)[5:9], "_Ind1"))

# d2 <- merge(ibd[,c("Individ2","ii","exp.rel","obs.rel")], 
#             scanAnnot2[c("scanName","sex", "race", "Case_Control", "Disease_Status", "APOE")], 
#             by.x = "Individ2", by.y = "scanName")

# colnames(d2) <- c("Individ2", "ii", "exp.rel", "obs.rel", paste0(colnames(d)[5:9], "_Ind2"))
