...
diag.geno <- createDataFile(path=path_to_data, geno.file, file.type="gds",
                            ...
                            sep.type="\t",
                            skip.num=1, col.total=7, col.nums=col.nums,
                            scan.name.in.file=1, 
...
# create an R image for the diagnostic file
diag.qxy.file <- "./GWAS3_diag.qxy.RData"

# Assign the column number to the corresponding feilds and name
col.nums <- as.integer(c(1,2,5))
names(col.nums) <- c("snp", "sample", "quality")
...
# create an R image for the diagnostic file
diag.bl.file <- "./GWAS3_diag.bl.RData"

# Assign the column number to the corresponding feilds and name
col.nums <- as.integer(c(1,2,3,4))
