...
# snpAnnot@data[ind,]
# snpID   Index   snpName     chromosome  position  rsID        missing.n1
# 820806  1611882 rs77900513  8           13976430  rs13265807  1
...
#   0.0004656 0.0008897 0.0012201 0.0033391 0.0019125 0.4825251

# Look at missing.e1 for females
summary(scanAnnot$missing.e1[scanAnnot$sex == "F"])
#       Min.   1st Qu.    Median      Mean   3rd Qu.      Max. 
#   0.0004253 0.0008615 0.0011773 0.0024895 0.0018886 0.5686079 
...
summary(missa)
#       Min.   1st Qu.    Median      Mean   3rd Qu.      Max. 
#   0.0004251 0.0008364 0.0011227 0.0028724 0.0017923 0.5686631 

# X chromosome
missx <- miss$missing.counts[,"X"] / miss$snps.per.chr["X"]
summary(missx)
#       Min.   1st Qu.    Median      Mean   3rd Qu.      Max. 
#   0.0003139 0.0013539 0.0020210 0.0045393 0.0034927 0.5753277 
...
scanAnnot <- scanAnnot[order(scanAnnot$GEMID, scanAnnot$missing.e1),]
scanAnnot$duplicated <- duplicated(scanAnnot$GEMID)
...
#      Min.   1st Qu.    Median      Mean   3rd Qu.      Max. 
#   0.0000000 0.0000000 0.0003586 0.0019543 0.0007171 1.0000000 
...
#     Min.   1st Qu.    Median      Mean   3rd Qu.      Max. 
# 0.0001646 0.0003120 0.0004475 0.0021139 0.0008709 0.5677555 
...
# bn         1 3.771e-06 3.7715e-06  0.9768 0.3314
# Residuals 28 1.081e-04 3.8609e-06  
...
# Exclude 13 high missing call rate samples !!!
exc.qual.results <- qual.results[!row.names(qual.results)%in%scan.exclude,]
# qual.results[row.names(qual.results)%in%scan.exclude,]
# scanID  mean.quality      median.quality
# 68      0.7427358         0.8296
# 184     0.7257238         0.8192
# 186     0.6990662         0.8011
# 661     0.6949901         0.7947
# 665     0.7213974         0.8163
# 1582    0.6894673         0.7894
# 2168    0.7311819         0.8232
# 2387    0.6241527         0.6958
# 2636    0.7520676         0.8355
# 2739    0.7065330         0.8046
# 2741    0.7514216         0.8349
...
# Find out numbers of bad bins per sample & also exclude those 13 high missing call rate samples 
...
# exclude those 11 high missing call rate samples
...
boxplot(miss.rate$X ~ scanAnnot$sex[-scan.exclude],
...
boxplot(scanAnnot$het.A[scanAnnot$scanID%nin%scan.exclude] ~ scanAnnot$race[scanAnnot$scanID%nin%scan.exclude],
...
ind <- which(scanAnnot$sex == "F" & scanAnnot$scanID%nin%scan.exclude)
...
### Exame results ###
# range(scanAnnot$het.A[scanAnnot$scanID%nin%scan.exclude])
...
# Remove 11 hihg missing call rate 
...
for(i in 1:max(as.integer(mean_inten_2$plate))){
  
  # Assign each sex a color for each plate
  temp <- subset(mean_inten_2, plate==i)
...
# anootation file without 11 bad samples 
...
ind1 <- which(mean_inten_2$X>1 & mean_inten_2$sex=="M")
ind2 <- which(mean_inten_2$Y>0.5 & mean_inten_2$sex=="F")

ind <- sort(c(ind1, ind2))

mis_sex <- subset(mean_inten_2[ind,], select=c("corrected_scanName","sex","race","age","edu","center",
                                               "plate","well_position"))
...
ind3 <- which(mean_inten_2$het.X<0.05 & mean_inten_2$sex=="F")
ind4 <- ind3[ind3%nin%ind]

mis_1x <- subset(mean_inten_2[ind4,], select=c("corrected_scanName","sex","race","age","edu","center",
                                               "plate","well_position"))
...
cols[samp$race == "White"] <- "blue"
cols[samp$race == "Black"] <- "red"
cols[samp$race == "Other"] <- "black"
cols[samp$race == "Asian/PacificIslander"] <- "green"
...
princomp$case.ctrl.status <- as.factor(samples.nodup$AD)
...
table(samples.nodup$AD)

# Assign color codes for AD and Control
cols <- rep(NA, nrow(samples.nodup))
cols[samples.nodup$AD == "AD"] <- "green"
cols[samples.nodup$AD == "Control"] <- "magenta"
...
lm.all <- lm(scanAnnot$missing.e1 ~ scanAnnot$AD)
summary(aov(lm.all))

#                     Df  Sum Sq   Mean Sq F value Pr(>F)
# scanAnnot$AD        1   0.0003 0.0002821   0.743  0.389
# Residuals         2797 1.0625 0.0003799          

# Plot Missing call rate by status
pdf("./QC_check/Ch6.2-1 Missing call rate by status.pdf")
boxplot(scanAnnot$missing.e1[-scan.exclude] ~ scanAnnot$AD[-scan.exclude], 
...
baf.low.qual <- anomIdentifyLowQuality(snp.annot = snpAnnot, med.baf.sd, baf.seg.info,
...
dupdisc <- duplicateDiscordance(genoData, subjName.col="GEMID",
...
summary(dat$disc)
#         Min.   1st Qu.    Median      Mean   3rd Qu.      Max. 
#   3.436e-06 9.463e-06 1.950e-05 5.843e-05 6.817e-05 6.174e-04 
...
# dis>0 1.019483e-03 1.014841e-02 9.699346e-02 6.403171e-01
# dis>1 5.096593e-07 5.066042e-05 4.771567e-03 2.688179e-01
# dis>2 1.664763e-10 1.653679e-07 1.547043e-04 8.072784e-02
# dis>3 3.990655e-14 3.967259e-10 3.698050e-06 1.851090e-02
# dis>4 0.000000e+00 7.457198e-13 6.936687e-08 3.390686e-03
# dis>5 0.000000e+00 1.183936e-15 1.062297e-09 5.122691e-04
# dis>6 0.000000e+00 4.217194e-17 1.364959e-11 6.535070e-05
# dis>7 0.000000e+00 4.070378e-17 1.501192e-13 7.165743e-06
...
elig_subj <- scanAnnot$scanID[!scanAnnot$duplicated & !(scanAnnot$scanName %in% c("6209831")) & scanAnnot$missing.e1 < 0.05 ]
# 51 duplicates, 11 missing.e1 >= 0.05
...
vcfWrite(genoData, vcf.file="GEM_GWAS_raw.vcf", sample.col="corrected_scanName",
         id.col="rsID", qual.col=NULL, filter.cols=NULL,
...
save.image("./QC_check/GEM_GWAS_QC.RData")
# load("./QC_check/GEM_GWAS_QC.RData")

# d1 <- merge(ibd[,c("Individ1","ii","exp.rel","obs.rel")], 
#             scanAnnot2[c("scanName","sex", "race", "age", "AD", "dementia", "plate","well_position", "edu", "center", "cdr", "weight", "height", "bmi")], 
#             by.x = "Individ1", by.y = "scanName")

# colnames(d1) <- c("Individ1", "ii", "exp.rel", "obs.rel", paste0(colnames(d1)[5:17], "_Ind1"))

# d2 <- merge(ibd[,c("Individ2","ii","exp.rel","obs.rel")], 
#             scanAnnot2[c("scanName","sex", "race", "age", "AD", "dementia", "plate","well_position", "edu", "center", "cdr", "weight", "height", "bmi")], 
#             by.x = "Individ2", by.y = "scanName")

# colnames(d2) <- c("Individ2", "ii", "exp.rel", "obs.rel", paste0(colnames(d2)[5:17], "_Ind2"))
