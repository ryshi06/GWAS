...
# length(intersect(d$scanName, eigenvec$IID))

eigenvec_race <- merge(eigenvec, d[,c("scanName","race_recode")], by.x = "IID", by.y = "scanName")
...
white_inBlack <- merge(white_inBlack[,c("IID", "PC1", "PC2")], d, by.x = "IID", by.y = "scanName")
dim(white_inBlack)
...
black_inWhite <- merge(black_inWhite[,c("IID", "PC1", "PC2")], d, by.x = "IID", by.y = "scanName")
dim(black_inWhite)
...
asian_inWhite <- merge(asian_inWhite[,c("IID", "PC1", "PC2")], d, by.x = "IID", by.y = "scanName")
dim(asian_inWhite)
...
write.table(problematic, "./QC_Check/AllCohorts_AllChr_Race_Discrepancies.txt", 
