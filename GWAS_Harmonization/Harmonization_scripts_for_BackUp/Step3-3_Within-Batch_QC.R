...
load("Batch_Annotation.RData")

# assign file name and the path of the gds files to an object 
geno.file <- "./Batch_geno.gds"
qxy.file <- "./Batch_qxy.gds"
bl.file <- "./Batch_bl.gds"
...
write.table(ibd2, "./QC_check/Batch_GWAS_Kinship_Estimate.txt", quote=FALSE, row.names = FALSE, sep="\t")
...
#   race[i] <- scanAnnot$race[scanAnnot$BatchID == subjID[i]][1]
# }
...
### before running the following code, check genotype-based duplicated pairs first
save.image("./QC_check/Batch_QC.RData")
# load("./QC_check/Batch_QC.RData")
...
### extra samples excluded are one from genotype-based duplicated pairs with wrong phenotype information
### if the phenotype information cannot be confirmed, delete both samples from the duplicated pair
...
vcfWrite(genoData, vcf.file="Batch_raw.vcf", sample.col="scanName",
